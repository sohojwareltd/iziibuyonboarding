    ...
    <body class="bg-brand-neutral">
            ...
            <div class="p-4 md:p-8">
                <div class="bg-brand-neutral p-4 md:p-8">
                    <div class="max-w-[1200px] mx-auto">
                        ...
                        <div
                            class="bg-white border border-gray-200 rounded-t-xl p-4 md:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-100">
                                        @forelse($documentTypes as $docType)
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-6 py-4">
                                                    <div class="font-medium text-gray-900">{{ $docType->document_name }}
                                                    </div>
                                                </td>
                                                ...
                                                <td class="px-6 py-4">
                                                    <div class="flex flex-wrap gap-1 mb-1">
                                                        @foreach ($docType->allowed_file_types as $type)
                                                            <span
                                                                class="bg-gray-100 border border-gray-200 text-gray-600 px-2 py-0.5 rounded text-[10px] font-medium">{{ strtoupper($type) }}</span>
                                                        @endforeach
                                                    </div>
                                                    <div class="text-[10px] text-gray-400">Max {{ $docType->max_file_size }}
                                                        MB</div>
                                                </td>
                                                <td class="px-6 py-4">
                                                    @if ($docType->sensitivity_level === 'highly-sensitive')
                                                        <span
                                                            class="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs font-medium">Highly
                                                            Sensitive</span>
                                                ...
                                                <td class="px-6 py-4 text-center">
                                                    ...
                                                    @else
                                                        <span
                                                            class="bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full text-xs font-medium">Inactive</span>
                                                    @endif
                                                </td>
                                                <td class="px-6 py-4 text-right">
                                                    <div class="flex items-center justify-end gap-2">
                                                        <button onclick="editDocumentType({{ $docType->id }})"
                                                            class="text-gray-400 hover:text-brand-primary p-2"><i
                                                                class="fa-solid fa-pen text-sm"></i></button>
                                                        <button
                                                            onclick="deleteDocumentType({{ $docType->id }}, '{{ $docType->document_name }}')"
                                                            class="text-gray-400 hover:text-red-500 p-2"><i
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="6" class="px-6 py-8 text-center text-gray-500">No document
                                                    types found. Click "Add Document Type" to create one.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
            ...
            <div class="flex flex-col h-full">
                <div class="border-b border-gray-200 px-6 py-5">
                    ...
                    <button onclick="closeDrawer()"
                        class="absolute top-5 right-6 text-gray-400 hover:text-gray-600 p-2 rounded-full hover:bg-gray-100"><i
                    ...
                    <form id="document-type-form" class="space-y-8" data-mode="create" data-id="">
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="document-name" name="document_name" class="form-input"
                                    placeholder="e.g., Passport, Business License" required>
                            </div>
                            <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Min Pages <span
                                            class="text-gray-400 text-xs font-normal">(Optional)</span></label>
                                    <input type="number" id="min-pages" name="min_pages" class="form-input"
                            ...
                            <div class="space-y-2.5">
                                <label class="flex items-center gap-2.5 cursor-pointer">
                                    <input type="checkbox" id="visible-merchant" name="visible_to_merchant"
                                        value="1" class="w-3.5 h-3.5 border-gray-400 rounded text-blue-600" checked>
                                ...
                                <label class="flex items-center gap-2.5 cursor-pointer">
                                    <input type="checkbox" id="visible-admin" name="visible_to_admin" value="1"
                                        class="w-3.5 h-3.5 border-gray-400 rounded text-blue-600" checked>
                                    <span class="text-sm text-gray-700">Visible to Admin</span>
                                </label>
                                <label class="flex items-center gap-2.5 cursor-pointer">
                                    <input type="checkbox" id="mask-metadata" name="mask_metadata" value="1"
                                        class="w-3.5 h-3.5 border-gray-400 rounded">
                        ...
                        <div class="space-y-4 border-t border-gray-200 pt-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Acquirer <span
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-2">Solution <span
                                        class="text-gray-400 text-xs font-normal">(Multi-select)</span></label>
                        ...
                        <div class="space-y-4 border-t border-gray-200 pt-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Internal Notes <span
                                        class="text-gray-400 text-xs font-normal">(Optional)</span></label>
                                <textarea id="internal-notes" name="internal_notes" class="form-input resize-y" rows="2"
                                    placeholder="For compliance team usage only..."></textarea>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="border-t border-gray-200 px-6 py-4 flex items-center justify-between bg-white">
                    <button onclick="closeDrawer()"
                        class="text-brand-secondary font-medium hover:text-brand-primary flex items-center gap-2"><i
                            class="fa-solid fa-xmark text-sm"></i><span>Cancel</span></button>
                    <button id="save-btn"
                        class="bg-brand-accent text-white px-5 py-2.5 rounded-lg font-medium shadow-sm hover:bg-orange-500 transition-colors flex items-center gap-2"><i
                            class="fa-solid fa-check text-sm"></i><span>Save Document Type</span></button>
                </div>
            </div>
        </div>
        ...
        <div id="delete-modal"
            ...
            <div class="bg-white rounded-xl shadow-2xl max-w-md w-full animate-scale-in">
                <div class="p-6">
                    ...
                    <p class="text-gray-600 text-center mb-6">Are you sure you want to delete <span id="delete-doc-name"
                            class="font-semibold text-brand-primary"></span>? This action cannot be undone.</p>
                    <div class="flex gap-3">
                        ...
                        <button id="confirm-delete-btn"
                            class="flex-1 px-4 py-3 bg-red-600 text-white rounded-lg font-medium hover:bg-red-700 transition-colors shadow-sm"><i
                                class="fa-solid fa-trash-can mr-2"></i>Delete</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            document.getElementById('document-type-form').addEventListener('submit', function(e) {
                ...
                const acquirers = Array.from(document.querySelectorAll('input[name="required_acquirers"]:checked')).map(
                    chk => chk.value);
                const countries = Array.from(document.querySelectorAll('input[name="required_countries"]:checked')).map(
                    chk => chk.value);
                const solutions = Array.from(document.querySelectorAll('input[name="required_solutions"]:checked')).map(
                    chk => chk.value);
                const fileTypes = Array.from(document.querySelectorAll('input[name="allowed_file_types"]:checked')).map(
                    chk => chk.value);
                ...
                cleanData.append('visible_to_merchant', document.getElementById('visible-merchant').checked ? '1' :
                '0');
