                        ...
                        <div class="bg-white border border-gray-200 border-t-0 rounded-b-xl shadow-sm overflow-hidden">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="@if($method->category == 'card') bg-blue-100 text-blue-700 @elseif($method->category == 'wallet') bg-purple-100 text-purple-700 @else bg-green-100 text-green-700 @endif px-2.5 py-0.5 rounded-full text-xs font-medium capitalize">{{ $method->category }}</span>
                                        </td>
                                        <td class="px-6 py-4">
                                            @if($method->supported_countries && count($method->supported_countries) > 0)
                                                <span class="text-gray-600 text-sm">{{ $method->supported_countries[0] }}@if(count($method->supported_countries) > 1) +{{ count($method->supported_countries) - 1 }}@endif</span>
                                            @else
                                            ...
                                            <div class="flex gap-1 flex-wrap">
                                                @if($method->supported_acquirers && count($method->supported_acquirers) > 0)
                                                    @foreach(array_slice($method->supported_acquirers, 0, 2) as $acq)
                                                        <span class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-xs">{{ $acq['name'] ?? '' }}</span>
                                                    @endforeach
                                                    @if(count($method->supported_acquirers) > 2)
                                                        <span class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-xs">+{{ count($method->supported_acquirers) - 2 }}</span>
                                                    @endif
                                                @else
                                            ...
                                            <div class="flex gap-1 flex-wrap">
                                                @if($method->supported_solutions && count($method->supported_solutions) > 0)
                                                    ...
                                                    @if(count($method->supported_solutions) > 2)
                                                        <span class="border border-gray-200 text-gray-600 px-2 py-0.5 rounded text-xs">+{{ count($method->supported_solutions) - 2 }}</span>
                                                    @endif
                                                @else
                                                    <span class="text-gray-400 text-xs">—</span>
                                                @endif
                                        ...
                                        <td class="px-6 py-4">
                                            <span class="@if($method->is_active) bg-green-100 text-green-700 @else bg-red-100 text-red-700 @endif px-2.5 py-0.5 rounded-full text-xs font-medium">{{ $method->is_active ? 'Active' : 'Inactive' }}</span>
                                        </td>
                            ...
                            <div class="bg-gray-50 border-t border-gray-200 px-6 py-4 flex items-center justify-between">
                                <div class="text-xs text-gray-500">
                                    Showing <span class="font-medium text-gray-900">{{ ($paymentMethods->currentPage() - 1) * $paymentMethods->perPage() + 1 }}</span> to <span class="font-medium text-gray-900">{{ min($paymentMethods->currentPage() * $paymentMethods->perPage(), $paymentMethods->total()) }}</span> of <span class="font-medium text-gray-900">{{ $paymentMethods->total() }}</span> results
                                </div>
                    ...
                    <div class="space-y-8 flex-1">
                        ...
                        <div class="space-y-4 border-t border-gray-200 pt-4">
                            ...
                            <div>
                                ...
                                <div id="countries-container" class="border border-gray-200 rounded-lg p-2 flex flex-wrap gap-2 min-h-[42px]">
                            ...
                            <div id="acquirers-container" class="space-y-3">
                                ...
                                <div class="border border-gray-200 rounded-lg p-3">
                                    <label class="flex items-start gap-3 cursor-pointer">
                                        <input type="checkbox" class="acquirer-checkbox w-4 h-4 border-gray-400 rounded mt-0.5" value="{{ $acquirer->id }}" data-name="{{ $acquirer->name }}" onchange="toggleAcquirerTypes(this)">
                                        <div class="flex-1">
                                            ...
                                            <div class="flex gap-4 ml-7 acquirer-types" data-acquirer-id="{{ $acquirer->id }}" style="display: none;">
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="checkbox" class="acquirer-type-checkbox w-3 h-3 border-gray-400 rounded" data-acquirer-id="{{ $acquirer->id }}" value="online">
                                                    <span class="text-xs text-gray-600">Online</span>
                                                </label>
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="checkbox" class="acquirer-type-checkbox w-3 h-3 border-gray-400 rounded" data-acquirer-id="{{ $acquirer->id }}" value="pos">
                                                    <span class="text-xs text-gray-600">POS</span>
                                                </label>
                                                <label class="flex items-center gap-2 cursor-pointer">
                                                    <input type="checkbox" class="acquirer-type-checkbox w-3 h-3 border-gray-400 rounded" data-acquirer-id="{{ $acquirer->id }}" value="recurring">
            ...
            <div class="bg-white rounded-xl shadow-2xl max-w-md w-full animate-scale-in">
                <div class="p-6">
                    ...
                    <p class="text-gray-600 text-center mb-6">
                        Are you sure you want to delete <span id="delete-payment-method-name" class="font-semibold text-brand-primary"></span>? This action cannot be undone.
                    </p>
                    <div class="flex gap-3">
...
@push('scripts')
    <script>
        ...
        function resetForm() {
            ...
            supportedCountries = [];
            document.getElementById('countries-container').innerHTML = '<input type="text" id="country-input" placeholder="Add country..." class="flex-1 min-w-[120px] border-0 outline-none text-sm bg-transparent" onkeypress="handleCountryInput(event)">';
            
            // Reset all toggles
            ['supports_3ds', 'allows_tokenization', 'requires_additional_documents', 'requires_acquirer_configuration'].forEach(field => {
        ...
        function updateSupportedAcquirers() {
            ...
            document.querySelectorAll('.acquirer-checkbox:checked').forEach(checkbox => {
                ...
                document.querySelectorAll(`.acquirer-type-checkbox[data-acquirer-id="${acquirerId}"]:checked`).forEach(typeCheckbox => {
        ...
        document.getElementById('payment-method-form').addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    window.location.href = `{{ route('admin.masters.payment-method-master') }}?success=${encodeURIComponent(successMessage)}`;
                } else {
                    if (data.errors) {
